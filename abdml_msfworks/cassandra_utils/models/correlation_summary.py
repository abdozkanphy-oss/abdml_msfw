...
#     __table_name__ = "production_correlation"
#     workstation_id = columns.Integer(partition_key=True)
#     output_stock_id = columns.Text(partition_key=True)
#     process_id = columns.Text(primary_key=True, clustering_order="ASC")
#     quantity = columns.Double(required=True)
#     process_timestamp = columns.DateTime(required=True)
...
#             # Handle timestamp
#             try:
#                 timestamp = message.get('createDate', datetime.now(pytz.UTC))
#                 if isinstance(timestamp, (int, float)):
#                     # Convert milliseconds to seconds
#                     timestamp = datetime.fromtimestamp(timestamp / 1000, tz=pytz.UTC)
#                 elif not isinstance(timestamp, datetime):
#                     ph3_1_logger.error(f"Invalid createDate type: {type(timestamp)}, using current time")
...
#             )
#             ph3_1_logger.debug(f"Saved ProductionCorrelation: {record.process_id}")
#             return record
...
#     __table_name__ = "production_correlation_summary"
...
#                 ph3_1_logger.error(f"Invalid timestamp: {timestamp} (must be offset-aware)")
#                 return None
...
#                     ph3_1_logger.warning(f"Naive first_production: {existing_first}, assuming UTC")
#                     existing_first = existing_first.replace(tzinfo=pytz.UTC)
#                 if existing_last.tzinfo is None:
#                     ph3_1_logger.warning(f"Naive last_update: {existing_last}, assuming UTC")
#                     existing_last = existing_last.replace(tzinfo=pytz.UTC)
